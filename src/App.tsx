...
const App: React.FC = () => {
  const memoriesCtx = useContext(MemoriesContext); // we want to load the images on load when opening the app.

  const { initContext } = memoriesCtx;
  useEffect(() => { // along with useCallback in initContext in MemoriesContextProvider, initContext will not be recreated. 
    // Therefore, will not run into an infinite loop.
    initContext();
  }, [initContext]);
